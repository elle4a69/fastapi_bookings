...
import { CategoryDetail, ServiceDetail, UiError } from '../../types';
...
export const CategoryManager: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<CategoryDetail>>({});
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError(err.uiError || { code: 'FETCH_FAILED', message: 'Could not load categories.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelect = (category: CategoryDetail) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      let saved: CategoryDetail;
      if (selectedId) {
        saved = await adminService.updateCategory(selectedId, formData);
        setCategories(prev => prev.map(c => c.id === saved.id ? saved : c));
      } else {
        saved = await adminService.createCategory(formData);
        setCategories(prev => [...prev, saved]);
        setSelectedId(saved.id);
      }
      setFormData(saved);
      setIsEditing(false);
      setSaveMessage('Category saved.');
      setTimeout(() => setSaveMessage(null), 4000);
    } catch (err: any) {
      setError(err.uiError || { code: 'SAVE_FAILED', message: 'We couldn\'t save this category. Your changes are still here.' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`md:w-1/3 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden ${selectedId || isEditing ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {isLoading && categories.length === 0 ? (
            ...
            <div className="p-4 text-center text-slate-500 text-sm">No categories yet. Add a category to group your services.</div>
          ) : (
            categories.map(category => (
              <button
                key={category.id}
                onClick={() => handleSelect(category)}
                className={`w-full text-left px-4 py-3 rounded-lg text-sm transition-colors ${selectedId === category.id ? 'bg-indigo-50 text-indigo-900 font-medium' : 'hover:bg-slate-50 text-slate-700'}`}
              >
                <div className="flex justify-between items-center">
                  <span>{category.name}</span>
                  {!category.active && <span className="text-xs bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded">Inactive</span>}
                </div>
              </button>
            ))
          )}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center">
                ...
                <h2 className="font-bold text-slate-800">{selectedId ? 'Edit Category' : 'New Category'}</h2>
              </div>
              {!isEditing && !isStaff && (
                ...
                {isEditing && (
                  <div className="pt-6 flex justify-end space-x-3">
                    <button
                      ...
                      onClick={() => {
                        if (selectedId) {
                          handleSelect(categories.find(c => c.id === selectedId)!);
                        } else {
                          setSelectedId(null);
                          setIsEditing(false);
                        }
