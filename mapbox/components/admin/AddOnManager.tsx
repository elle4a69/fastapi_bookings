...
import { AddOnDetail, ServiceDetail, UiError } from '../../types';
...
export const AddOnManager: React.FC = () => {
  ...
  const [addOns, setAddOns] = useState<AddOnDetail[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AddOnDetail>>({});

  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [addOnData, svcData] = await Promise.all([
      ...
      setError(err.uiError || { code: 'FETCH_FAILED', message: 'Could not load add-ons.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelect = (addOn: AddOnDetail) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      let saved: AddOnDetail;
      if (selectedId) {
        saved = await adminService.updateAddOn(selectedId, formData);
        setAddOns(prev => prev.map(a => a.id === saved.id ? saved : a));
      } else {
        saved = await adminService.createAddOn(formData);
        setAddOns(prev => [...prev, saved]);
        setSelectedId(saved.id);
      }
      setFormData(saved);
      setIsEditing(false);
      setSaveMessage('Add-on saved.');
      setTimeout(() => setSaveMessage(null), 4000);
    } catch (err: any) {
      setError(err.uiError || { code: 'SAVE_FAILED', message: 'We couldn\'t save this add-on. Your changes are still here.' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`md:w-1/3 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden ${selectedId || isEditing ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {isLoading && addOns.length === 0 ? (
            ...
            <div className="p-4 text-center text-slate-500 text-sm">No add-ons yet. Add an optional extra to a service.</div>
          ) : (
            addOns.map(addOn => (
              <button
                key={addOn.id}
                onClick={() => handleSelect(addOn)}
                className={`w-full text-left px-4 py-3 rounded-lg text-sm transition-colors ${selectedId === addOn.id ? 'bg-indigo-50 text-indigo-900 font-medium' : 'hover:bg-slate-50 text-slate-700'}`}
              >
                <div className="flex justify-between items-center">
                  <span>{addOn.name}</span>
                  {!addOn.active && <span className="text-xs bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded">Inactive</span>}
                </div>
              </button>
            ))
          )}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center">
                ...
                <h2 className="font-bold text-slate-800">{selectedId ? 'Edit Add-on' : 'New Add-on'}</h2>
              </div>
              {!isEditing && !isStaff && (
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700">Added Duration (mins)</label>
                      <input
                        type="number"
                        min="0"
                        required
                        disabled={!isEditing || isLoading}
                        value={formData.duration_minutes !== undefined ? formData.duration_minutes : ''}
                        onChange={e => setFormData({...formData, duration_minutes: parseInt(e.target.value, 10)})}
                        className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-3 py-2 border disabled:bg-slate-50 disabled:text-slate-500"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700">Added Price ($)</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                ...
                {isEditing && (
                  <div className="pt-6 flex justify-end space-x-3">
                    <button
                      ...
                      onClick={() => {
                        if (selectedId) {
                          handleSelect(addOns.find(a => a.id === selectedId)!);
                        } else {
                          setSelectedId(null);
                          setIsEditing(false);
                        }
