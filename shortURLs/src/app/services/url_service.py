...
class URLShortener:
    def __init__(self, base_url: str | None = None):
        ...
        self.base_url = (base_url or settings.short_base_url).rstrip("/")
    ...
    def add_url(self, long_url: str) -> str:
        with self.lock:
            ...
            if long_url in self.url_to_id:
                return self.build_short_url(self.url_to_id[long_url])
            short_path = self.generate_short_code()
            self.url_to_id[long_url] = short_path
            self.id_to_url[short_path] = long_url

            return self.build_short_url(short_path)

    def get_long_url(self, short_url: str) -> str | None:
        if not short_url.startswith(f"{self.base_url}/"):
            return None

        short_path = short_url.rstrip("/").split("/")[-1]
